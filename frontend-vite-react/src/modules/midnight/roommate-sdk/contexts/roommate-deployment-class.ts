import {
  type RoommateProviders,
  RoommatePrivateStateId,
} from "../api/common-types";
import { type ContractAddress } from "@midnight-ntwrk/compact-runtime";
import { BehaviorSubject } from "rxjs";
import { type Logger } from "pino";
import { type LocalStorageProps } from "./roommate-localStorage-class";
import {
  ContractController,
...
export class DeployedTemplateManager implements DeployedAPIProvider {
  constructor(
    ...
    private readonly providers?: RoommateProviders
  ) {}
  ...
  private async deploy(
    ...
    try {
      if (this.providers) {
        ...
        console.error("Deploy failed: Providers not available");
        deployment.next({
          status: "failed",
