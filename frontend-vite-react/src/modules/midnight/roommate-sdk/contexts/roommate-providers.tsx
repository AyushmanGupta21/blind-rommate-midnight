...
import type {
  RoommateCircuits,
  RoommatePrivateStateId,
} from "../api/common-types";
import { RoommateProviders } from "../api/common-types";
...
import { RoommateMatchPrivateState } from "@eddalabs/roommate-contract";
import {
  fromHex,
  toHex,
...
export interface ProvidersState {
  privateStateProvider: PrivateStateProvider<typeof RoommatePrivateStateId>;
  zkConfigProvider?: ZKConfigProvider<RoommateCircuits>;
...
export const Provider = ({ children, logger }: ProviderProps) => {
  ...
  const privateStateProvider: PrivateStateProvider<
    typeof RoommatePrivateStateId
  > = useMemo(
    () =>
      inMemoryPrivateStateProvider<string, RoommateMatchPrivateState>(),
    [logger, status]
  );
  ...
  const zkConfigProvider = useMemo(() => {
    ...
    return new CachedFetchZkConfigProvider<RoommateCircuits>(
      `${window.location.origin}/midnight/rommate-match`,
      fetch.bind(window),
      () => {}
    );
  }, [status]);
